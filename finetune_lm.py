...
def load_mask(model, mask_path):
    state_dict_mask = torch.load(mask_path)
    param_tensor = model.state_dict()
    for key in state_dict_mask:
...
def apply_n_m_sparsity_to_model(model, n, m):
    ...
    masks_dict = {}
    decoder_layers = model.base_model.model.model.decoder.layers
...
def main():
    ...
    if model_args.config_name:
        ...
        config = AutoConfig.from_pretrained("decapoda-research/llama-7b-hf", **config_kwargs)
    ...
    if model_args.model_name_or_path:
        ...
        # model = AutoModelForCausalLM.from_pretrained("decapoda-research/llama-7b-hf", torch_dtype=torch.float16, cache_dir="/project_data/llm_weights/", low_cpu_mem_usage=True, device_map="auto") #, **config_kwargs)
        # load_mask(model, model_args.model_name_or_path)
    else:
